...
PHP_FUNCTION(tc_tgetent) {
  ...
  TRACE_CUSTOM("calling tgetent with terminal %s", buffer);
  
  result = tgetent(NULL, buffer);
  if (result == 1) {
    TRACE_CUSTOM("returned true from tc_tgetent");
    RETURN_TRUE;
  } else if (result == 0) {
    TRACE_CUSTOM("returned false from tc_tgetent");
    RETURN_FALSE;
  } else {
    TRACE_CUSTOM("returned null from tc_tgetent");
    RETURN_NULL();
  }
}

PHP_FUNCTION(tc_tgetnum) {
  char* buffer;
  int buffer_len;
  int result;
  ...
  TRACE_CUSTOM("calling tgetnum with %s", buffer);
    
  result = tgetnum(buffer);
  
  TRACE_CUSTOM("got %d from tgetnum", result);
